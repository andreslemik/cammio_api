...
module CammioAPI
  # Describes a specific video answer. 
  class Answer
    attr_accessor :question_text

    attr_accessor :hls_url
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CammioAPI::Answer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CammioAPI::Answer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'hls_url')
        self.hls_url = attributes[:'hls_url']
      end

      if attributes.key?(:'dash_url')
        self.dash_url = attributes[:'dash_url']
      end
    ...
    def hash
      [question_text, hls_url, dash_url, thumb_url, still_url].hash
    end
