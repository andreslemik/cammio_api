...
module CammioAPI
  # Add the description for a Pitch. 
  class Pitch
    attr_accessor :id

    attr_accessor :title

    attr_accessor :invite_hash
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CammioAPI::Pitch` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CammioAPI::Pitch`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'invite_hash')
        self.invite_hash = attributes[:'invite_hash']
      end
    ...
    def hash
      [id, title, invite_hash, language].hash
    end
